...
import {
  ...
  detectRedFlags,
  calculateKMin,
  calculateFullTaskSuccessProbability,
  DEFAULT_CONFIG,
  formatSubtaskPrompt,
} from './maker-core.js';
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    ...
    case 'maker_decompose_task': {
      const { task, customSteps } = args as { task: string; customSteps?: string[] };
    ...
    case 'maker_check_red_flags': {
      const { response, maxTokens } = args as { response: string; maxTokens?: number };
    ...
    case 'maker_submit_vote': {
      const { subtaskId, response } = args as { subtaskId: string; response: string };
    ...
    case 'maker_calculate_k': {
      const { totalSteps, successRate = 0.95, targetProbability = 0.95 } = args as {
        totalSteps: number;
        successRate?: number;
      ...
      const fullTaskProb = calculateFullTaskSuccessProbability(
        totalSteps,
        successRate,
        result.kMin
      );
